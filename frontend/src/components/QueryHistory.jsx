export default function QueryHistory({ history, loading, onSelect }) {
  if (loading) return <div className="p-4 text-[#767E8C] font-mono text-[11px]">Loading query log...</div>;
    ...
    <div className="h-full flex flex-col font-mono text-[11px]">
      ...
      <div className="flex-1 overflow-y-auto p-2 divide-y divide-[#141C2B]/10">
        {!history || history.length === 0 ? (
          <div className="p-4 text-center text-[#767E8C] text-[11px] italic">No historical queries recorded</div>
        ) : (
          history.map(item => (
            <button
              key={item.id}
              onClick={() => onSelect(item.id)}
              className="w-full text-left p-2.5 hover:bg-[#EAE3D5] transition-colors group flex flex-col gap-1"
            >
              <div className="flex items-center justify-between w-full">
                ...
                <span className="text-[#767E8C] text-[10px]">
                  {new Date(item.timestamp || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
              <div className="text-[#141C2B] font-medium text-[11px] line-clamp-2">
                {item.question}
              </div>
            </button>
          ))
