...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-full flex flex-col p-8 sm:p-12 max-w-6xl mx-auto space-y-12 pb-24">
      
      {/* Hero Header Area */}
      <div className="space-y-4 pt-4 pb-4 border-b border-[#141C2B]/15">
        <p className="text-[12px] font-mono tracking-[0.14em] uppercase text-[#767E8C]">
          System Status: Ready · Database Live · 6 Tables Available
        </p>
        <h1 className="font-serif text-5xl sm:text-6xl font-medium tracking-tight text-[#141C2B] leading-[1.12]">
          Ask your data anything.<br />
          We'll <em className="italic text-[#2C4A8F] font-serif">clarify</em> before we query.
        </h1>
        <p className="text-[#4A5364] text-[15px] sm:text-[16px] leading-[1.8] max-w-3xl font-mono">
      ...
      {/* Query Input Box */}
      <div className="bg-[#E5DED0] border border-[#141C2B]/20 p-8 shadow-sm">
        <QueryInput onSubmit={askQuestion} isLoading={isLoading} />
      </div>

      {/* Error Banner */}
      {error && (
        <div className="bg-[#FAF2EE] border-l-4 border-red-700 p-6 flex gap-4 text-[#141C2B]">
          ...
          <div className="space-y-1 text-sm">
            ...
            <p className="font-mono text-[#4A5364] text-[14px]">{error}</p>
          </div>
        </div>
      )}
      ...
      {/* Clarification Step */}
      {step === 'clarifying' && (
        <div className="border border-[#2C4A8F]/40 bg-[#E8E2D5] p-8 shadow-sm">
          <ClarificationDialog 
      ...
      {isLoading && step !== 'asking' && (
        <div className="border border-[#141C2B]/20 bg-[#E5DED0] p-10 text-center space-y-4">
          <div className="inline-block w-8 h-8 border-2 border-[#2C4A8F] border-t-transparent animate-spin"></div>
          <p className="font-mono text-[14px] uppercase tracking-[0.1em] text-[#141C2B]">
